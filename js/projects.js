...
const buildType_fields     = 'fields=build(id,buildTypeId,number,branchName,status,webUrl,finishOnAgentDate,statusText,failedToStart,problemOccurrences,testOccurrences(testOccurrence(status,newFailure,muted,test(investigations(investigation(assignee))))),buildType(investigations(investigation(id,state,assignee,assignment,scope,target))))'
